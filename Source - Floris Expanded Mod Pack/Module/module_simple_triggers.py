	...
	[
		(try_for_range, ":town_no", towns_begin, towns_end),							#	Floris	//	Adjust Population Depending on Prosperity
			...
			(else_try),
				...
				(val_sub, ":change", 3),
			(try_end),
			(try_begin), #F&B allow higher pop, but at slower growth rate
				(ge, ":population", 40000),
				(val_div, ":change", 4),
			(else_try),
			    (ge, ":population", 30000),
...
##    ]),
  ...
  (24,
   [
       (try_for_range, ":village_no", villages_begin, villages_end),
       	 (party_slot_eq, ":village_no", slot_party_type, spt_village),				#F&B Check fix to prevent castles spawning farmer parties. But it shouldn't occur in the first place
        ...
        (try_begin),
         ...
         (else_try),
           ...
           (else_try),
		     ...
		     (ge, ":merchant_faction", 1),
		     (neq, ":merchant_faction", "fac_kingdom_100"),
		     #F&b bug hunting end
...
####################################################################################################################################
	...
	(24, ## Note: faction and town_lord need to be set upon construction. So script build_village can find it
		[
		(eq, 1, 0), #F&B temp Disabled
		
		(try_for_range, ":center_no", ruins_begin, ruins_end),
			(party_slot_eq, ":center_no", slot_in_construction, 1),
			(store_current_day, ":cur_day"),			
			(party_get_slot, ":day_started", ":center_no", slot_construction_started),
			(store_sub, ":days_passed", ":cur_day", ":day_started"),
			(ge, ":days_passed", 30),     #1 month construction time, before build village
			(assign, ":new_center", villages_end),			
			#(call_script, "script_clone_party", ":center_no", villages_end),
            #(val_add, "$g_villages_end", 1),
            (party_get_slot, s7, ":center_no", slot_orig_name),
            (party_set_name, ":new_center", s7),			#Set name for build village early to avoid bug with "ruins of ruins of xxx" name
            #(call_script, "script_destroy_village", ":center_no"),   #Cannot use this to destroy ruins!
            (call_script, "script_build_village", 0, ":new_center"),   
            (try_for_range, reg1, 0, 1000), 
	            (party_set_slot, ":center_no", reg1, 0),     #Set all slots to 0
	        (try_end),
	        ...
	        #(val_add, "$g_ruins_begin", 1),		#F&B Fix currently ruins don't get reorganized. So more and more ruins until no more fiefs.
		(try_end),
		]
		),
	#Trigger to send worker parties to construction site 
		(60, 
			[
			(eq, 1, 0),	#F&B temp disabled
			
			(try_for_range, ":center_no", centers_begin, centers_end),
			    (party_get_slot, ":construction_no", ":center_no", slot_center_supporting_construction),
			    (ge, ":construction_no", 1),   #center is supporting construction   
                (store_faction_of_party, ":party_faction", ":center_no"),  
                (set_spawn_radius, 0),
				##Floris MTT begin
				(try_begin),
		 			(eq, "$troop_trees", troop_trees_0),
					(spawn_around_party, ":center_no", "pt_village_farmers"),
				(else_try),
		 			(eq, "$troop_trees", troop_trees_1),
					(spawn_around_party, ":center_no", "pt_village_farmers_r"),
				(else_try),
					(eq, "$troop_trees", troop_trees_2),
					(spawn_around_party, ":center_no", "pt_village_farmers_e"),
				(try_end),
				##Floris MTT end
              (assign, ":new_party", reg0),
              (party_set_faction, ":new_party", ":party_faction"),              
              (party_set_slot, ":new_party", slot_party_type, spt_workers),
              (party_set_slot, ":new_party", slot_party_ai_state, spai_trading_with_town),
              (party_set_slot, ":new_party", slot_party_ai_object, ":construction_no"),
              ######### Determine target pos of workers, so they set camp around construction####
              (party_get_position, pos1, ":construction_no"),	#global position
              (position_get_x, ":x_pos", pos1),
              (position_get_y, ":y_pos", pos1),
              (assign, ":x_pos_move", 1),	#Dont change! Use multiplier at end to change dist from pos1
              (assign, ":y_pos_move", 1),
              	#Calculating local x,y-shift
              (try_begin),		#1 O'Clock pos 
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 0),              	               	               	  
                  (val_div, ":x_pos_move", 2),
                  (store_sqrt, ":y_pos_move", 3),
                  (val_div, ":y_pos_move", 2),  
                  (party_set_slot, ":center_no", slot_center_parties_sent, 1),                                                  
              (else_try),		#2 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 1),  
                  (store_sqrt, ":x_pos_move", 3),
                  (val_div, ":x_pos_move", 2),
                  (val_div, ":y_pos_move", 2),                  
                  (party_set_slot, ":center_no", slot_center_parties_sent, 2),
              (else_try),       #3 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 2),  
              	  (assign, ":y_pos_move", 0),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 3),
              (else_try),		#4 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 3),  
              	  (store_sqrt, ":x_pos_move", 3),
              	  (val_div, ":x_pos_move", 2),
              	  (val_div, ":y_pos_move", -2),   
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 4),
              (else_try),		#5 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 4),  
              	  (val_div, ":x_pos_move", 2),
              	  (store_sqrt, ":y_pos_move", 3),
              	  (val_div, ":y_pos_move", -2),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 5),
              (else_try),		#6 O'CLock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 5),  
              	  (assign, ":x_pos_move", 0),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 6),
              (else_try),		#7 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 6),  
              	  (val_div, ":x_pos_move", -2),
              	  (store_sqrt, ":y_pos_move", 3),
              	  (val_div, ":y_pos_move", -2),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 7),
              (else_try),		#8 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 7),  
              	  (store_sqrt, ":x_pos_move", 3),
              	  (val_div, ":x_pos_move", -2),
              	  (val_div, ":y_pos_move", -2),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 8),
              (else_try),		#9 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 8),  
              	  (assign, ":y_pos_move", 0),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 9),
              (else_try),		#10 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 9),  
              	  (store_sqrt, ":x_pos_move", 3),
              	  (val_div, ":x_pos_move", -2),
              	  (val_div, ":y_pos_move", 2),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 10),
              (else_try),		#11 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 10),  
              	  (val_div, ":x_pos_move", -2),
              	  (store_sqrt, ":y_pos_move", 3),
              	  (val_div, ":y_pos_move", 2),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 11),
              (else_try),		#12 O'Clock pos
              	  (party_slot_eq, ":center_no", slot_center_parties_sent, 11),  
              	  (assign, ":x_pos_move", 0),
              	  (party_set_slot, ":center_no", slot_center_parties_sent, 12),
              (try_end),
              ## multiplier to adjust final distance from construction site
              (val_mul, ":x_pos_move", 3),
              (val_mul, ":y_pos_move", 3),
              ## making global pos from the local position shifts
              (val_add, ":x_pos", ":x_pos_move"),
              (val_add, ":y_pos", ":y_pos_move"),
              (position_set_x, pos2, ":x_pos"),
              (position_set_y, pos2, ":y_pos"),
              ## safety check. Only proceed, if pos is valid terrain
              (spawn_around_party, ":center_no", "pt_village_farmers"), 
              (assign, ":test", reg1),                                        
              (party_get_current_terrain, ":terrain", ":test"),
              (remove_party, ":test"),
              (try_begin),	
                  ...
                  (eq, ":terrain", 9),
                  (map_get_land_position_around_position, pos2, pos1, 3),	#get random valid position, if other pos was invalid terrain                  
              (try_end),
              (party_set_ai_behavior, ":new_party", ai_bhvr_travel_to_point),
              (party_set_ai_target_position, ":new_party", pos2),          
              (party_set_flags, ":new_party", pf_default_behavior, 0), 
			    
			(try_end),
			]
			),
	#Check, whether lords want to upgrade fiefs
	(24*7,
		[
		#(neq|eq, "$g_dynamic_ai", 0),
		(eq, "$g_dynamic_ai", 0),
...
#				(party_get_slot, ":party_no", ":center_no", slot_pass_party),
#				(remove_party, ":party_no"),
#				(set_spawn_radius, 0),
#				(spawn_around_party, ":center_no", "pt_merchant_caravan"),
#				#(party_clear, reg0),				
#				#(call_script, "script_party_add_party_companions", reg0, ":center_no"),
#				(store_faction_of_party, ":faction", ":center_no"),
#				(str_store_party_name, s1, ":center_no"),
#				(party_set_slot, reg0, slot_pass_defender, 1),
...
#				(party_set_slot, ":center_no", slot_pass_party, reg0),	
#				(party_set_slot, reg0, slot_pass_defender, ":center_no"),	
#			(try_end),
#			]),
...
#F&B Checks for finished constructions and replaces with the actual settlements
	(24*7, 
		...
		(try_for_range, ":center_no", villages_begin, villages_end),
			(party_slot_eq, ":center_no", slot_in_construction, 1),
			(party_slot_eq, ":center_no", slot_construction_type, spt_future_town),			# Safety. In case we add something like downgrade delay 
			
			(party_get_slot, ":start_day", ":center_no", slot_construction_started),
			(party_get_slot, ":build_time", ":center_no", slot_construction_time),
			(store_current_day, ":cur_day"),
            ...
            (call_script, "script_build_town", ":center_no"),
        (try_end),
...
### slot_quest_dont_give_again_period
	...
	# Downgrade trigger. For easy integration this is a seperate trigger, but could easily be intergrated into weekly budget scripts, to avoid another (try_for_range)
 (24*7,
 	[
 	(try_for_range, ":cur_center", centers_begin, centers_end),
 		(this_or_next|party_slot_eq, ":cur_center", slot_party_type, spt_village),
 		(party_slot_eq, ":cur_center", slot_party_type, spt_town),
 		
 		(party_get_slot, ":prosperity", ":cur_center", slot_town_prosperity),
 		(lt, ":prosperity", 15),
 		(party_get_slot, ":duration", ":cur_center", slot_center_low_prosperity_duration),
 		(val_add, ":duration", 7),
 		(party_set_slot, ":cur_center", slot_center_low_prosperity_duration, ":duration"),
 		(try_begin),
 			(party_slot_eq, ":cur_center", slot_party_type, spt_town),
 			(ge, ":duration", 90),
 			(call_script, "script_downgrade_town", ":cur_center"),
 		(else_try),
 			(party_slot_eq, ":cur_center", slot_party_type, spt_village),
 			(ge, ":duration", 70),
 			(call_script, "script_destroy_village", ":cur_center"),
 		(try_end),
 	(try_end),
 	]),

### Trigger to update Progress bar on construction sites 
(24, 
	[
	(try_for_parties, ":party_no"),
		(party_is_active, ":party_no"),
		(party_slot_eq, ":party_no", slot_in_construction, 1),
		(party_get_slot, ":build_time", ":party_no", slot_construction_time),
		(party_get_slot, ":start_time", ":party_no", slot_construction_started),
		(party_get_slot, ":future_spt", ":party_no", slot_construction_type),
		(store_current_day, ":cur_day"),
		(store_sub, ":passed_time", ":cur_day", ":start_time"),
		(store_div, ":completion", ":passed_time", ":build_time"),
		(store_mul, ":completion_percentage", ":completion", 100),
		(str_clear, s1),
		(try_begin),
			(eq, ":future_spt", spt_town),
			#(assign, reg1, 1),
			(str_store_string, s1, "@town"),
	    (else_try),
	    	(eq, ":future_spt", spt_castle),
	    	#(assign, reg1, 2),
	    	(str_store_string, s1, "@castle"),
	    (else_try),
	    	(eq, ":future_spt", spt_village),
	    	#(assign, reg1, 3),
	    	(str_store_string, s1, "@village"),
	    (try_end),
		(try_begin),
		    (lt, ":completion_percentage", 10),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[          ] 0%"),
		(else_try),
		    (lt, ":completion_percentage", 20),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[=         ] 1O%"),
		(else_try),
		    (lt, ":completion_percentage", 30),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[==        ] 2O%"),
		(else_try),
		    (lt, ":completion_percentage", 40),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[===       ] 3O%"),
	    (else_try),
		    (lt, ":completion_percentage", 50),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[====      ] 4O%"),	
		(else_try),
		    (lt, ":completion_percentage", 60),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[=====     ] 5O%"),
		(else_try),
		    (lt, ":completion_percentage", 70),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[======    ] 6O%"),
		(else_try),
		    (lt, ":completion_percentage", 80),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[=======   ] 7O%"),
	    (else_try),
		    (lt, ":completion_percentage", 90),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[========  ] 8O%"), 
		(else_try),
		    (lt, ":completion_percentage", 100),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[========= ] 9O%"),
		(else_try),
		    (le, ":completion_percentage", 100),
		    (party_set_extra_text, ":party_no", "@{s1} construction^[==========] 100%"),                       
		(try_end),
	...
	(24*7, 
		[### Simple trigger, which increases relation of village to nearby castle faction, if no own faction owned castle/town is nearby.

	(try_for_range, ":cur_village", villages_begin, villages_end),
		 (store_faction_of_party, ":village_faction", ":cur_village"),
         (assign, ":min_dist", 10),
         (assign, ":min_dist_castle", -1),
         (assign, ":min_dist_faction", -1),
         
         (assign, ":end_cond", castles_end),
         (try_for_range, ":cur_center", castles_begin, ":end_cond"),
         	 (assign, ":proceed", 1),
             (store_distance_to_party_from_party, ":cur_dist", ":cur_village", ":cur_center"), 
             (lt, ":cur_dist", 10),
             (store_faction_of_party, ":center_faction", ":cur_center"),
             ############### F&B Fix. this block does not do work. 
             (try_begin),
            	 (eq, ":village_faction", ":center_faction"),			#If there is a nearby friendly walled_center, break.
            	 (assign, ":end_cond", ":cur_center"),
            	 (assign, ":proceed", 0),
            	 (try_for_range, ":cur_faction", kingdoms_begin, kingdoms_end),			#if nearby friendly castle, drop relation to other factions twice as fast.
            	 	(store_sub, ":offset", ":cur_faction", kingdoms_begin),
        		    (store_add, ":vil_fac_rltn_slot", slot_village_rltn_to_fac_1, ":offset"),
            	 	(party_get_slot, ":fac_relation", ":cur_village", ":vil_fac_rltn_slot"),
            	 	(val_sub, ":fac_relation", 2),
            	 	(val_clamp, ":fac_relation", 0, 100),
            	 	(party_set_slot, ":cur_village", ":vil_fac_rltn_slot", ":cur_faction"),
            	 (try_end),
             (else_try),
             	# (assign, ":proceed", 1),
	             (lt, ":cur_dist", ":min_dist"),
	             (assign, ":min_dist", ":cur_dist"),
	             (assign, ":min_dist_castle", ":cur_center"),
	             (assign, ":min_dist_faction", ":center_faction"),
             (try_end),
         (try_end),
         (ge, ":proceed", 1),
         (party_set_slot, ":cur_village", slot_village_protected_by, ":min_dist_castle"),
         (store_sub, ":offset", ":min_dist_faction", kingdoms_begin),
         (store_add, ":vil_fac_rltn_slot", slot_village_rltn_to_fac_1, ":offset"),
         (party_get_slot, ":village_relation", ":cur_village", ":vil_fac_rltn_slot"),
         (val_add, ":village_relation", 1),
         (party_set_slot, ":cur_village", ":vil_fac_rltn_slot", ":village_relation"),
         (ge, ":village_relation", 20),
         (party_get_slot, ":player_relation", ":cur_village", slot_center_player_relation),
         (lt, ":player_relation", 50),		# Player villages with relation 50+ will never transfer.
         (assign, ":town_lord", -1),
         (party_get_slot, ":town_lord", ":min_dist_castle", slot_town_lord),
         (try_begin), #If lord exists, assign village to him, otherwise to faction
            (ge, ":town_lord", 1),				#Don't make automatic transfer for player. 
            (call_script, "script_give_center_to_lord", ":cur_village", ":town_lord"),
         (else_try),
            (neg|party_slot_eq, ":cur_center", slot_town_lord, "trp_player"), #Exclude player #F&B return this would be a good place to start a village conversion quest
            (call_script, "script_give_center_to_faction_aux", ":cur_village", ":min_dist_faction"),
         (try_end),
         (try_for_range, ":cur_slot", slot_village_rltn_to_fac_begin, slot_village_rltn_to_fac_end), 		#reset relation, to prevent fief swapping. (maybe unnecessary)
             (party_set_slot, ":cur_village", ":cur_slot", 0),
         (try_end),
     (try_end),
     ]),
# The End
