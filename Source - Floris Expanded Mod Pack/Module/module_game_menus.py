...
#from dsettlements_presentations import *
...
("start_game_options", 0,
 ...
 and that whatever course you take, great adventures will await you.",
	...
	[
		...
		("start_mod",[
      #(eq, 1, 0),        #F&B disable for release
      ],"Quick Start",
			[
				(call_script, "script_ccp_default_settings"),
				...
				(party_set_name, "p_main_party", "@Testers very own personal army"),
...
("start_phase_2",mnf_disable_all_keys,   # start_phase_2_5
    ...
    [
      ...
      (change_screen_return),
			#(jump_to_menu, "mnu_start_phase_3"),    #F&B skip merchant quest
		   ]),
    ]),
...
#TEMPERED  CHANGES END
    ...
    #F&B begin Dynamic World
      ...
      ("camp_build_village", 
        [(eq, "$g_build_village", 1)],
        "This is a fine place for a new village",
        [
        (quest_get_slot, ":orig_center", "qst_construct_settlement", slot_quest_constructing_center),
        (call_script, "script_construct_settlement", ":orig_center", spt_village),
        (assign, "$g_build_village", 0),
        ],
        ),
      ("camp_build_castle", 
        [(eq, "$g_build_castle", 1)],
        "This is a fine place for a new castle",
        [
        (quest_get_slot, ":orig_center", "qst_construct_settlement", slot_quest_constructing_center),
        (call_script, "script_construct_settlement", ":orig_center", spt_castle),
        (assign, "$g_build_castle", 0),
        ],
        ),
      ("camp_build_town", 
        [(eq, "$g_build_town", 1)],
        "This is a fine place for a new town",
        [
        (quest_get_slot, ":orig_center", "qst_construct_settlement", slot_quest_constructing_center),
        (call_script, "script_construct_settlement", ":orig_center", spt_town),
        (assign, "$g_build_town", 0),
        ],
        ),
      #F&B begin
      ("fb_cheat",
   ...
   [
    ...
    [
	  ...
	  (neq,"$cheat_mode",4)],"{!}Cheat: Set Debug messages to Political Only.",
	   ...
	   ###F&B begin     
      ("cheat_faction_orders",[(ge,"$cheat_mode",1)],
    "{!}Cheat: Set Debug messages to NONE.",
       [(assign,"$cheat_mode",0),
         (jump_to_menu, "mnu_camp_cheat"),
        ]
       ),
...
##diplomacy end+
    ...
    [
      ...
      "Advance to town",
      [
      (call_script, "script_build_town", "$g_encountered_party"),
      (change_screen_return),      
      ]),
    ("cheat_destroy",
      [(eq, "$cheat_mode", 1)],
      "Burn this village down to the ground",
      [
        (call_script, "script_destroy_village", "$g_encountered_party"),
        (change_screen_map),                       
        ]),
    ("cheat_rename",
      [     
    ...
    ("upgrade_menu",
      [
        (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_town),
       # (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (party_slot_eq, "$g_encountered_party", slot_town_lord, "trp_player"),
        (quest_slot_eq, qst_construct_settlement, slot_quest_constructing_center, "$g_encountered_party"),
      ],
      "Village upgrade menu",
      [(jump_to_menu, "mnu_construction")]),
    #F&B Dynamic World End      
      ("village_leave",[],"Leave...",
        [
        (change_screen_return,0),
		## CC 1.322
...
#           (party_add_template, "$current_town", "pt_villagers_in_raid"),
  ...
  #F&B begin Dynamic World
	("take_over_village", 
    [],
    "Try to convice the village to join your faction",        #F&B return. Should make this more transparent. Maybe like dialog "convince lord to join faction"
    [

    (party_get_slot, ":relation", "$current_town", slot_center_player_relation),
    (store_random_in_range, ":rand", 50, 70),
    (try_begin),
        (ge, ":relation", ":rand"),
        (party_get_slot, ":town_lord", "$current_town", slot_town_lord),
        (party_get_slot, ":rltn_lord", ":town_lord", slot_troop_player_relation),
        (val_sub, ":rltn_lord", 25),
        (party_set_slot, ":town_lord", slot_troop_player_relation, ":rltn_lord"),
        (str_clear, s1),
        (str_clear, s1),
        (str_store_party_name, s1, ":town_lord"),
        (str_store_party_name, s2, "$current_town"),
        (display_message, "@{s1} is furious about your actions. Your relationship has deteriorated by 25."),
        (call_script, "script_give_center_to_lord", "$current_town", "trp_player"),
        (display_message, "@You have successfully convinced the village {s2} to join your faction."),
    (else_try),
        (display_message, "@The village refuses to join your faction. Maybe if your relation was better, they could be persuaded."),
    (try_end),
    ]),
  #F&B end Dynamic World
	("forget_it",[],
...
#Lazeras MODIFIED (Training Yard)
       #F&B end
      ("center_build_improved_horses",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_improved_horses, 0),
                                       ],
       "Build stables.",[(assign, "$g_improvement_type", slot_center_has_improved_horses),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ...
      ("center_build_guild_hall",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
...
# #NPC companion changes end
 ...
 and then put the buildings to the torch. From the coins and valuables that are found, you get your share of {reg1} denars.",
    ...
    [
		...
		## WINDYPLAINS+ ## - Consequences of raiding a village. (Silverstag Specific)
       ...
       [
		  ...
		  (troop_clear_inventory, "trp_temp_troop"),
			...
			(try_for_range, ":trade_item", trade_goods_begin, trade_goods_end),
				...
				# (item_get_value, ":item_value", ":trade_item"), # WSE specific.
				(store_item_value, ":item_value", ":trade_item"),
...
#Floris End	
    ...
    ("fb_town_cheats",
      ...
      "Dynamic World Cheats",
      [(jump_to_menu, "mnu_fb_town_cheats_2"),
      ]),
    ("cheat_rename",
      [     
      (party_slot_eq, "$current_town", slot_party_type, spt_castle),      
...
#Tempered entrenched camp menu not currently used
 ...
 AI Behaviour is {s13}{reg18? (currently {s14}):}, Object is {s15}{reg19? (currently {s16}):} at ({reg20},{reg21})",
    ...
    [
      ...
      ("dist",
      [],
      "store distance from player party to here",
      [(store_distance_to_party_from_party, ":dist", "p_main_party", "$g_encountered_party"),
       (assign, reg1, ":dist"),
       (display_message, "@{reg1} ingame units"),
      ]),
...
#       ),
  ...
  #DEBUG OSP end
  #F&B begin Dynamic World Town Cheat Menu
  ("fb_town_cheats_2",0,
    "Behold the wonders of infinite power and use it wisely",     #F&B Fix! this cheat menu is broken. Probably only up&downgrading works
    "none",                                                       # build_scripts got rewritten. Need to adjust menu for player. For AI it should already work
    [
    ],
    [
    ...
    [
      ...
      [
      (call_script, "script_downgrade_town", "$g_encountered_party"),
      (change_screen_return),
      ]),
    ("cheat_build_village",
      [
    ...
    #F&B end Dynamic World Town Cheats  ]
  #F&B begin Dynamic World Camp Cheat Menu
  ("fb_cheat",0,
  ...
  ("fb_cheat_build_village",[], "Build new village here",
    ...
    (call_script, "script_build_village", -1),
    (change_screen_map),    
    (start_presentation, "prsnt_name_new_fief"),
   # (change_screen_return),
     ]
  ),
  ...
  ("fb_cheat_ai",[], "Run Dynamic Settlement AI Script",
    [(call_script, "script_dynamic_settlements_ai"),
     (call_script, "script_dynamic_settlements_ai_village"),
    (change_screen_return),
    ]),
  ("fb_script_ds_ai", [(neq|eq, "$g_dynamic_ai", 1),], "Disable Dynamic Settlement AI ",
  [
  (assign, "$g_dynamic_ai", 1),
  (change_screen_return),
  ]),
  ("fb_script_ds_ai", [(eq, "$g_dynamic_ai", 1),], "Enable Dynamic Settlement AI ",
  [
  (assign, "$g_dynamic_ai", 0),
  (change_screen_return),
  ]),
  ...
  #F&B begin ruin menu
  ("ruin",mnf_scale_picture,
   "{s9}",
   "none",
   [
     #(party_get_slot, ":construction_in_progress", "$g_encountered_party", slot_in_construction),      
     (str_store_party_name, s8, "$g_encountered_party"),
     (try_begin),
          (party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
          (str_store_string, s9, "@Construction is at work in the {s8}. You can see people building houses, all kinds of simple tools, even some early fences surrounding what will soon become fields.^The people are busy, but full of energy and you feel a glow of excitement."), 
     (else_try),
          (play_track, "track_retreat",1),
          (set_background_mesh, "mesh_pic_looted_village"),    
          (str_store_string, s9, "@You come across the {s8}. A once great town, now reduced to rubble. You can still make out some building foundations, however the ruins seem to have been abondened a long time ago. There is no one here anymore"),
     (try_end),         
   ],
    [
      ("visit_ruin",[],"Visit the ruins (No scenes yet, but allows rebuild quest)",
       [
       #(party_get_slot, ":construction_in_progress", "$g_encountered_party", slot_in_construction),
        (party_set_slot, "$g_encountered_party", slot_ruin_visited, 1),       #1 enables village elder/guild master talk option to rebuild
      ...
      ("qst_give_tools", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_tools_provided", 0),
        (call_script, "script_get_troop_item_amount", "trp_player", "itm_trade_tools"),
        (assign, ":cur_amount", reg0),
        (ge, ":cur_amount", 10),
        ],"Supply the required 10 tools.",
        [(troop_get_inventory_capacity, ":inv_size", "trp_player"),
          ...
          (try_for_range, ":i_slot", 0, ":inv_size"),
            ...
            (try_end),
            (troop_remove_item, "trp_player", "itm_trade_tools", 1), ##Floris 2.54 bugfix - missing the "trp_player" argument
            (val_sub, ":quest_target_amount", 1),
            (lt, ":quest_target_amount", 1),
            (assign, ":inv_size", 0), #break
          (try_end),
          (assign, "$g_tools_provided", 1),          
          (change_screen_return),
          ]),
      ("qst_give_money", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_money_provided", 0),  
        (store_troop_gold, ":wealth", "trp_player"),
        (ge, ":wealth", 25000),                                                 
        ],"Supply the required 25k.",
        [
        (troop_remove_gold, "trp_player", 25000),      
        (assign, "$g_money_provided", 1), 
        (change_screen_return),                      
        ]),
      ("qst_give_guards", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_guards_provided", 0),
        (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
        (lt, ":guard_size", 30),
                    
        #(store_party_size_wo_prisoners, ":party_size", "p_main_party"),
        #(gt, ":party_size", 1),                            
        ],"Provide a guard of at least 30 men.",
        [
        #(party_clear, "p_temp_party"),
        #(assign, "$g_move_heroes", 1),
      ...
      ("qst_give_guards2", 
        ...
        (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
        (ge, ":guard_size", 30),        
        ], "Have the guards start patrolling around here!",
        [
        
        (spawn_around_party, "$g_encountered_party", "pt_patrol_party", 5),
        (assign, ":guards", reg1),        
        #(party_clear, ":guards"),
        (call_script, "script_party_add_party", ":guards", "$g_encountered_party"),
        #(party_set_ai_object, ":guards", "$g_encountered_party"),
        (party_set_ai_behavior, ":guards", ai_bhvr_patrol_location),
        (store_faction_of_party, ":faction", "p_main_party"),        
        (party_set_faction, ":guards", ":faction"),        
        (assign, "$g_guards_provided", 1),    
        (change_screen_return),
      ...
      ("qst_give_cloth", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_cloth_provided", 0),
        (call_script, "script_get_troop_item_amount", "trp_player", "itm_trade_wool_cloth"),
        (assign, ":cur_amount", reg0),
        (ge, ":cur_amount", 10),        
        ],"Supply the required 10 cloth.",
        [(troop_get_inventory_capacity, ":inv_size", "trp_player"),
          (assign, ":quest_target_amount", 10),          
          (assign, ":result_xp_reward", 300), # Was 400
          (assign, ":result_prosperity", 3),  # Was 4
          ...
          (try_for_range, ":i_slot", 0, ":inv_size"),
            (troop_get_inventory_slot, ":cur_item", "trp_player", ":i_slot"),
            (eq, ":cur_item", "itm_trade_wool_cloth"),
            (troop_get_inventory_slot_modifier, ":cur_modifier", "trp_player", ":i_slot"),
            (try_begin),
              (eq, ":cur_modifier", imod_large_bag), # These men are starving, they only care about quantity, so no bonus or penalty for quality
              (val_add, ":result_xp_reward", 50),
              ...
              (val_add, ":result_relation", 1),
            (try_end),
            (troop_remove_item, "trp_player", "itm_trade_wool_cloth", 1), ##Floris 2.54 bugfix - missing the "trp_player" argument
            (val_sub, ":quest_target_amount", 1),
            (lt, ":quest_target_amount", 1),
            (assign, ":inv_size", 0), #break
          (try_end),
          (assign, "$g_cloth_provided", 1),          
          (change_screen_return),]),
      ("qst_give_tools", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_tools_provided", 1),
        ],"I've already provided tools",
        [(change_screen_return),]),
      ("qst_give_money", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_money_provided", 1),                        
        ],"I've already provided money",
        [(change_screen_return),]),
      ("qst_give_guards", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_guards_provided", 1),        
        ],"I've already provided a guard",
        [(change_screen_return),]),
      ("qst_give_cloth", [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        (eq, "$g_construction_quest", 1),
        (eq, "$g_cloth_provided", 1),        
        ],"I've already provided cloth",
        [(change_screen_return),]),
      ("qst_start", 
        [
        (party_slot_eq, "$g_encountered_party", slot_in_construction, 0),
        ...
        "Start construction",
        [
        #(call_script, "script_succeed_quest", "qst_rebuild"),
        (complete_quest, "qst_rebuild"),
        (party_set_slot, "$g_encountered_party", slot_in_construction, 1),
        (store_current_day, reg1),
        (party_set_slot, "$g_encountered_party", slot_construction_started, reg1), 
        (party_set_slot, "$g_encountered_party", slot_town_lord, "trp_player"),
        (store_faction_of_party, ":faction", "p_main_party"),
        (party_set_faction, "$g_encountered_party", ":faction"),      #F&B Check. Might be too early to assign to player faction
...
("construction",mnf_scale_picture,
   "{s1}{s2}{s3}{s4}{s5}{s6}{s7}{s14}{s10}{s11}{s12}{s13}{s15}",    #s8= lord name, s9=center name 
   "none",
   [
   (str_clear, s1),
   (str_clear, s2),
   ...
   #(party_get_slot, s9, "$g_encountered_party", slot_orig_name),
   #(str_clear, s9),
   #(str_store_string_reg, s9, reg9),
   (str_store_party_name, s9, "$g_encountered_party"),
   (assign, ":center_no", "$g_encountered_party"),
   (party_get_slot, ":spt", "$g_encountered_party", slot_construction_type),
   (party_get_slot, ":town_lord", "$g_encountered_party", slot_construction_lord),
   (party_get_current_terrain, ":terrain", "$g_encountered_party"),
   (str_store_troop_name, s8, ":town_lord"),
   
   (try_begin),
       (eq, ":town_lord", "trp_player"),
       (party_slot_eq, ":center_no", slot_in_construction, 0),
       (str_store_string, s1, "@You are visiting your own construction site of {s9}. Work has not yet started.^"),
   (else_try),
       (eq, ":town_lord", "trp_player"),
       (party_slot_eq, ":center_no", slot_in_construction, 1),
       (str_store_string, s1, "@You are visiting your own construction site of {s9}. With everything supplied, construction is now well on the way.^"),
   (else_try),
       (str_store_string, s1, "@You come across {s8}'s construction site of {s9}.^You can see people working hard."),
   (try_end),
  
   (try_begin),
       (eq, ":town_lord", "trp_player"),
       (eq, ":spt", spt_future_village),
       (neg|party_slot_eq, ":center_no", slot_in_construction, 1),
       (str_store_string, s2, "@You need to supply:^"),
       (str_store_string, s7, "@You have already supplied:^"),
       (try_begin),
           #(eq, "$g_supplied_guards", 0),
           (assign, ":min_size", 30),
           (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
           (store_sub, ":diff", ":min_size", ":guard_size"),
           (gt, ":diff", 0),           
           (assign, reg1, ":min_size"),
           (str_store_string, s3, "@An additional {reg1} guards to protect the construction site^"),
       (else_try),
           (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
           (assign, reg1, ":guard_size"),
           (ge, ":guard_size", 1),
           (str_store_string, s10, "@ {reg1} guards^"),
       (try_end),
       (try_begin),
           (eq, "$g_supplied_tools", 0),
           (str_store_string, s4, "@10 tools^"),
       (else_try),
           (str_store_string, s11, "@tools^"),
       (try_end),
       (try_begin),
           (eq, "$g_supplied_pickaxes", 0),
           (str_store_string, s5, "@2 pickaxes^"),
       (else_try),
           (str_store_string, s12, "@pickaxes^"),
       (try_end),
       (try_begin),
           (eq, "$g_supplied_denar", 0),
           (str_store_string, s6, "@25k denar^"),
       (else_try),
           (str_store_string, s13, "@enough denar^"),
       (try_end),
       (try_begin),
           (eq, "$g_supplied_cloth", 0),
           (try_begin),
               (this_or_next|eq, ":terrain", rt_snow),
               (eq, ":terrain", rt_snow_forest),
               (str_store_string, s14, "@ 3 furs"),
           (else_try),
               (this_or_next|eq, ":terrain", rt_desert),
               (eq, ":terrain", rt_desert_forest),
               (str_store_string, s14, "@1 raw silk"),
           (else_try),
               (str_store_string, s14, "@3 wool cloth"),
           (try_end),
       (else_try),
           (str_store_string, s15, "@enough clothing"),
       (try_end),       
       (try_begin),
           (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
           (eq, ":guard_size", 0),          
           ...
           (eq, "$g_supplied_pickaxes", 0),
           (eq, "$g_supplied_denar", 0),
           (str_store_string, s10, "@Nothing"),
        (try_end),
       
   (else_try),
       (eq, ":town_lord", "trp_player"),
       (eq, ":spt", spt_future_castle),
       (neg|party_slot_eq, ":center_no", slot_in_construction, 1),
       (str_store_string, s2, "@You need to supply:^"),
       (str_store_string, s7, "@You have already supplied:^"),
       (try_begin),
           ...
           (gt, ":diff", 0),           
           (assign, reg1, ":min_size"),
           (str_store_string, s3, "@{reg1} more guards to protect the construction site^"),
       (else_try),
           (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
           (assign, reg1, ":guard_size"),
           (ge, ":guard_size", 1),
   ...
   (else_try),
       ...
       (try_begin),
           (eq, "$g_supplied_pickaxes", 0),
           (str_store_string, s5, "@3 pickaxes^"),
       (else_try),
           (str_store_string, s12, "@pickaxes^"),
       (try_end),
       (try_begin),
           (eq, "$g_supplied_denar", 0),
           (str_store_string, s6, "@50k denar^"),
       (else_try),
           (str_store_string, s13, "@enough denar^"),
       (try_end),
              (try_begin),
  ...
  [
    ##### begin Village construction menus
    ("village_construction_1", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      #(eq, "$g_supplied_guards", 0),

      ],
      "guards (opens exchange menu)",
      [
      (try_begin),
        (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
       # (lt, ":guard_size", 30),
      ...
      (try_begin),
        (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
        (ge, ":guard_size", 30),
        (assign, "$g_supplied_guards", 1),
      (else_try),
        (assign, "$g_supplied_guards", 0),  
      (try_end),

      ]),
    ("village_construction_2", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      ...
      [
          ...
          (try_for_range, ":i_slot", 0, ":inv_size"),
            ...
            (eq, ":cur_item", "itm_trade_tools"),
           # (troop_get_inventory_slot_modifier, ":cur_modifier", "trp_player", ":i_slot"),
            (troop_remove_item, "trp_player", "itm_trade_tools", 1), 
            (val_sub, ":quest_target_amount", 1),
            (lt, ":quest_target_amount", 1),
            (assign, ":inv_size", 0), #break
          (try_end),
          (assign, "$g_supplied_tools", 1),
      ]),
    ("village_construction_3", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      (eq, "$g_supplied_pickaxes", 0),
      (call_script, "script_get_troop_item_amount", "trp_player", "itm_we_rho_axe_pick"),
      (assign, ":cur_amount", reg0),
      (ge, ":cur_amount", 2),   
      ],
      "Supply 2 pickaxes",
      [
          ...
          (try_for_range, ":i_slot", 0, ":inv_size"),
            (troop_get_inventory_slot, ":cur_item", "trp_player", ":i_slot"),
            (eq, ":cur_item", "itm_we_rho_axe_pick"),
            #(troop_get_inventory_slot_modifier, ":cur_modifier", "trp_player", ":i_slot"),
            (troop_remove_item, "trp_player", "itm_we_rho_axe_pick", 1), 
            (val_sub, ":quest_target_amount", 1),
            (lt, ":quest_target_amount", 1),
            (assign, ":inv_size", 0), #break
          (try_end),
          (assign, "$g_supplied_pickaxes", 1),
      ]),
    ("village_construction_4", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      ...
      [
      (troop_remove_gold, "trp_player", 25000),      
      (assign, "$g_supplied_denar", 1), 
      (change_screen_return),   
      ]),
    ("village_construction_5", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      ...
      (assign, ":show", 0),
           (try_begin),
               ...
               (call_script, "script_get_troop_item_amount", "trp_player", "itm_trade_furs"),
               (assign, ":cur_amount", reg0),
               (ge, ":cur_amount", 3),
               (str_store_string, s16, "@3 furs"),
               (assign, ":show", 1),
           (else_try),
               (this_or_next|eq, ":terrain", rt_desert),
               (eq, ":terrain", rt_desert_forest),
               (call_script, "script_get_troop_item_amount", "trp_player", "itm_trade_raw_silk"),
               (assign, ":cur_amount", reg0),
               (ge, ":cur_amount", 1),
               (str_store_string, s16, "@1 raw silk"),
               (assign, ":show", 1),
           (else_try),
               (call_script, "script_get_troop_item_amount", "trp_player", "itm_trade_wool_cloth"),
               (assign, ":cur_amount", reg0),
               (ge, ":cur_amount", 3),
               (str_store_string, s16, "@3 wool cloth"),
               (assign, ":show", 1),
           (try_end),
           (eq, ":show", 1),
      ],
      ...
      (party_get_current_terrain, ":terrain", "$g_encountered_party"),
           (try_begin),
               (this_or_next|eq, ":terrain", rt_snow),
               (eq, ":terrain", rt_snow_forest),
               (assign, ":cloth_type", "itm_trade_furs"),
           (else_try),
               (this_or_next|eq, ":terrain", rt_desert),
               (eq, ":terrain", rt_desert_forest),
               (assign, ":cloth_type", "itm_trade_raw_silk"),
           (else_try),
               (str_store_string, s16, "@3 wool cloth"),
               (assign, ":cloth_type", "itm_trade_wool_cloth"),
           (try_end),  
          (troop_get_inventory_capacity, ":inv_size", "trp_player"),
          (assign, ":quest_target_amount", 2),          
          (try_for_range, ":i_slot", 0, ":inv_size"),
            (troop_get_inventory_slot, ":cur_item", "trp_player", ":i_slot"),
            (eq, ":cur_item", ":cloth_type"),
            #(troop_get_inventory_slot_modifier, ":cur_modifier", "trp_player", ":i_slot"),
            (troop_remove_item, "trp_player", ":cloth_type", 1), 
            (val_sub, ":quest_target_amount", 1),
            (lt, ":quest_target_amount", 1),
            (assign, ":inv_size", 0), #break
          (try_end),
          (assign, "$g_supplied_cloth", 1),               
      ]),
    ##### end Village construction menus 
    ##### begin Castle construction menus 
("castle_construction_1", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_castle),
      #(eq, "$g_supplied_guards", 0),
    ...
    ("castle_construction_4", 
      ...
      "Pay 35k",
      [
      (troop_remove_gold, "trp_player", 35000),      
      (assign, "$g_supplied_denar", 1), 
      (change_screen_return),   
      ]),
("castle_construction_5", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_castle),
    ...
    ##### end Castle construction menus 
    ##### begin Town construction menus 
    ("town_construction_1", 
      [
      (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
      (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      ...
      "100 guards (opens exchange menu)",
      [
      (try_begin),
        (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
       # (lt, ":guard_size", 100),
...
("town_construction_5", 
    ...
    ##### end Town construction menus 
    ("start_construct",[
          #(assign, ":center_no", "$g_encountered_party"),
                #Start construction upon leaving, when all requirements fullfilled
           (party_slot_eq, "$g_encountered_party", slot_construction_lord, "trp_player"),
           (neg|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
           (eq, "$g_supplied_guards", 1),
           (eq, "$g_supplied_tools", 1),
           (eq, "$g_supplied_pickaxes", 1),
           (eq, "$g_supplied_denar", 1),

         ],"Start construction and leave.",
       [
           (assign, ":center_no", "$g_encountered_party"),
           (party_set_slot, ":center_no", slot_in_construction, 1),
           (assign, "$g_supplied_guards", 0),
           (assign, "$g_supplied_tools", 0),
           (assign, "$g_supplied_pickaxes", 0),
           (assign, "$g_supplied_denar", 0),
           (store_current_day, ":day"),
           (party_set_slot, ":center_no", slot_construction_started, ":day"),
           (party_get_slot, ":construction_time", ":center_no", slot_construction_time),
           (assign, reg1, ":construction_time"),
           (display_message, "@With all supplies ready, construction at {s9} has now begun.^ Estimated construction duration is {reg1} days."),
       (change_screen_return),],
       ),
      ("leave",[],"Leave.",
       [(change_screen_return),
