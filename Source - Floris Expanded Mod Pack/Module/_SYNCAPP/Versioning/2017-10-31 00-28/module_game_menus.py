...
#TEMPERED  CHANGES END
    ...
    #F&B begin Dynamic World
      ...
      ("camp_build_village", 
        ...
        "Start construction of a village here",
        [
      ...
      ("camp_build_castle", 
        [(eq, "$g_build_castle", 1)],
        "Start construction of a castle here",
        [
        (quest_get_slot, ":orig_center", "qst_construct_settlement", slot_quest_constructing_center),
      ...
      ("camp_build_town", 
        [(eq, "$g_build_town", 1)],
        "Start construction of a town here",
        [
        (quest_get_slot, ":orig_center", "qst_construct_settlement", slot_quest_constructing_center),
...
("construction",mnf_scale_picture,
   ...
   (else_try),
       ...
       (str_store_string, s1, "@You are visiting your own construction site of {s9}. Work is under way. People are busy.^"),
   (else_try),
   ...
   (try_begin),
       ...
       (neq|party_slot_eq, ":center_no", slot_in_construction, 1),
       ...
       (try_end),
       (try_begin),
           (eq, "$g_supplied_utilities", 0),
           (str_store_string, s5, "@2 pitch forks^"),
       (else_try),
...
#    - 25k ^\
  [
    ...
    ("village_construction_1", 
      ...
      (neq|party_slot_eq, "$g_encountered_party", slot_in_construction, 1),
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      #(eq, "$g_supplied_guards", 0),

      ],
      "30 guards (opens exchange menu)",
      [
      (try_begin),
        (store_party_size_wo_prisoners, ":guard_size", "$g_encountered_party"),
       # (lt, ":guard_size", 30),
    ...
    ("village_construction_3", 
      ...
      (party_slot_eq, "$g_encountered_party", slot_construction_type, spt_future_village),
      (eq, "$g_supplied_utilities", 0),
      (call_script, "script_get_troop_item_amount", "trp_player", "item_we_rho_spear_fork_pitch"),
      (assign, ":cur_amount", reg0),
      (ge, ":cur_amount", 2),   
      ],
      "Supply 2 pitch forks",
      [
          (troop_get_inventory_capacity, ":inv_size", "trp_player"),
          (assign, ":quest_target_amount", 2),          
          (try_for_range, ":i_slot", 0, ":inv_size"),
            (troop_get_inventory_slot, ":cur_item", "trp_player", ":i_slot"),
            (eq, ":cur_item", "item_we_rho_spear_fork_pitch"),
            #(troop_get_inventory_slot_modifier, ":cur_modifier", "trp_player", ":i_slot"),
            (troop_remove_item, "trp_player", "itm_we_rho_spear_fork_pitch", 1), 
            (val_sub, ":quest_target_amount", 1),
            (lt, ":quest_target_amount", 1),
            (assign, ":inv_size", 0), #break
          (try_end),
          (assign, "$g_supplied_utilities", 1),
      ]),
